...
## 图文绑定与位置
- 每个内容模块先确定容器 x/y/w/h，再在容器内安排图标区、标题区、正文区、标签区；不要让文字脱离所属卡片或图标乱飞
- 图标/贴纸必须和对应标题、说明形成清晰组合：图标通常居中在标题上方，或在左侧与标题垂直居中对齐；图标与文字至少间隔 16px
- 主标题和副标题不要压在同一位置；标题长时主动换行，副标题放下一行或右侧独立文本框，并与标题保持 18px 以上距离
- 底部推荐条、软件标签、页码要先预留固定区域，不要贴到画布底边，也不要覆盖主内容卡片
- 装饰图形放在背景层、角落或模块边缘；不要插入正文行间，不要压住任何 text 文本框
